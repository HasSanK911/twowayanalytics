...
interface ModalProps {
    isOpen: boolean;
    onClose: () => void;
    onOpenAddModal: () => void; // New function to open Add Modal

}
function editmodal({ isOpen, onClose,onOpenAddModal  }: ModalProps) {
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-[#00000052] bg-opacity-50 z-1000">
                <div className="bg-white p-6 rounded-lg shadow-lg max-w-[900px] w-full   ">
                    ...
                    <div className='flex gap-3 mt-4'>
                        <label htmlFor="" className="text-[#5A5A5A] text-xl mb-4">Publication ID:</label>
                        <h2 className="text-xl font-semibold">233-XYZ-3820</h2>
                    </div>
                    ...
                    <button className="bg-[#AEAEB2] flex items-center justify-center gap-2 btn text-[#ffffff] h-[47px] w-[98px] text-lg font-semibold rounded-lg" onClick={onClose}>  Cancel</button>
                        <button className="bg-[#1A3561] flex items-center justify-center gap-2 btn text-[#ffffff] h-[47px] w-[165px] text-lg font-semibold rounded-lg" onClick={onOpenAddModal}

                        ><img src={add} alt="filtericon" />  Add Post</button>
                    </div>
